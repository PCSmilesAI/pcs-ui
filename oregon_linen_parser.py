#!/usr/bin/env python3
"""
Oregon Linen Invoice Parser

Parses invoices from Oregon Linen Inc. (mat/linen rental service).
"""
...
def parse_invoice_date(text: str) -> Optional[str]:
    ...
    # Look for date in M/D/YYYY format
...
def parse_total(text: str) -> Optional[str]:
    ...
    match = re.search(r'Invoice Total:\s*\$?([\d,]+\.?\d*)', text, re.IGNORECASE)
    if match:
...
def parse_line_items(text: str) -> List[Dict[str, Any]]:
    ...
    # Pattern: Item#, Qty, Description, Price
    pattern = r'(\d+)\s+(\d+)\s+([A-Z0-9\s\-]+?(?:MAT|TREAD|TOWEL|GLOVE|MOPS))\s+\$([\d.]+)'
...
def parse(pdf_path: str) -> Dict[str, Any]:
    ...
    if 'oregon linen' not in text.lower():
        raise RuntimeError("Not an Oregon Linen invoice")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 oregon_linen_parser.py <pdf_path>")
        sys.exit(1)
