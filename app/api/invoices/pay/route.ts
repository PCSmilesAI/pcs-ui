  ...
  try {
    ...
    for (const invoiceId of invoiceIds) {
      try {
        ...
        // Tag the QBO bill with the batch ID for easy filtering in QBO search
        ...
        try {
          ...
          if (existingBill && existingBill.SyncToken && existingBill.VendorRef) {
            // Update the Memo field (searchable in QBO) with batch ID
            const currentMemo = existingBill.Memo || 'Bill Generated by PCS AI';
            ...
            const updatePayload = {
              ...
              Memo: newMemo, // Searchable field in QBO
            };

            await qboClient.updateBill(updatePayload);
            tagSuccess = true;
            taggedCount++;
            console.log('[PAYMENT] Tagged bill', invoice.qbo_bill_id, 'with batch ID in Memo:', batchId);
